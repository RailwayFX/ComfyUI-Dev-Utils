...
async function setExecutionTimeLoggingEnabled(enabled) {
    try {
        await api.fetchApi(`/ty-dev-utils/execution-time/logging`, {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({enabled})
        });
    } catch (e) {
        console.warn("Failed to update ExecutionTime logging setting", e);
    }
}
...
app.registerExtension({
    ...
    async setup() {
        ...
        const executionTimeLoggingEnabled = app.ui.settings.addSetting({
            id: "TyDev-Utils.ExecutionTime.Logging.Enabled",
            name: "TyDev ExecutionTime Logging Enabled",
            type: "boolean",
            defaultValue: false,
            onChange: setExecutionTimeLoggingEnabled
        });
        await setExecutionTimeLoggingEnabled(executionTimeLoggingEnabled);
